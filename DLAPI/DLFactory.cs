...
namespace DLAPI
{
    ...
    public static class DLFactory
    {
        ...
        public static IDL GetDal()
        {
            ...
            {
                throw new DLConfigException($"Failed loading {dalPackage}.dll", ex);
            }
            ...
            if (type == null)
                throw new DLConfigException($"Class name is not the same as Assembly Name: {dalPackage}");
            ...
            try
            {
                IDL dal = type.GetProperty("Instance", BindingFlags.Public | BindingFlags.Static).GetValue(null) as IDal;
                // If the instance property is not initialized (i.e. it does not hold a real instance reference)...
                if (dal == null)
                    throw new DLConfigException($"Class {dalPackage} instance is not initialized");
                // now it looks like we have appropriate dal implementation instance :-)
                return dal;
            }
            catch (NullReferenceException ex)
            {
                throw new DLConfigException($"Class {dalPackage} is not a singleton", ex);
            }
